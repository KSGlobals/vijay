...
export class DashboardComponent implements OnInit {
    ...
    this.firestore
      ...
      .subscribe((querySnapshot) => {
        querySnapshot.forEach((doc) => {
          const projectData = doc.data() as { SPOC: string[]; date: Date }; // Assuming 'spoc' is a string field

          if (projectData.SPOC[0] === this.userName) {
            this.companies.push(projectData);
            this.companydetails = projectData;
            console.log(this.companydetails.company);
            this.getProjectsCount(this.companydetails.company);
            this.getCompletedProjectsCount(this.companydetails.company);
            this.getOngoingProjectsCount(this.companydetails.company);
            this.getUpcomingProjectsCount(this.companydetails.company);
  ...
  getProjectsCount(company: any) {
    this.firestore
      .collection('Project', (ref) => ref.where('company', '==', company))
      .get()
      .subscribe((querySnapshot) => {
  ...
  getCompletedProjectsCount(company: any) {
    this.firestore
      .collection('Project', (ref) =>
        ref.where('status', '==', 'Completed').where('company', '==', company)
      )
      .get()
      .subscribe((querySnapshot) => {
  ...
  getCompanyData() {
    this.firestore
      ...
      .subscribe((querySnapshot) => {
        querySnapshot.forEach((doc) => {
          const projectData = doc.data() as {
            SPOC: string[];
            date: Date;
          }; // Assuming 'spoc' is a string field
